...
public class CRMessageRelMovJ extends BaseMessage {
    ...
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("RelMovJ("+offset1+","+offset2+","+offset3+","+offset4+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("RelMovJ("+offset1+","+offset2+","+offset3+","+offset4+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("RelMovJ("+offset1+","+offset2+","+offset3+","+offset4+")");
    }
    ...
    public double[]getOffset(){
        return new double[]{offset1,offset2,offset3,offset4};
    }
}
